...
1. 点击“打开图片”。初始 ROI 为整张图片。
2. 按住 `Shift` 拖拽重新框选；在框内拖拽移动；拖拽控制点缩放；也可输入 `x`、`y`、`width`、`height`。
3. 调整参数后点击“计算”。修改图片、ROI 或任一参数会使整份结果过期。
4. 切换视图或保存 PNG。`Ctrl` 加滚轮缩放画布；“适配窗口”不会放大超过原图尺寸。

任意时刻只运行一个后台分析。分析期间发生输入变化时，旧任务的成功或失败结果都会被
丢弃。左右对比视图共享 ROI、缩放和滚动位置。

## 协议与坐标契约

Explorer 使用 `NormalizedBBox.from_pixel_xywh()` 把整数 ROI 转为规范化 bbox，随后
...
GUI 和导出只显示落入所选 `pixel_bbox` 的证据。渲染先按 `content_bounds` 和
`valid_mask` 取出有效内容，再按 `transform.source_size` 恢复到原始
`context_bounds`，最后裁剪到 `pixel_bbox`。区域边界通过 floor/ceil 映回原图，与
`pixel_bbox` 求交后才转成 ROI 局部坐标；完整画布、padding 和 ROI 外上下文不会被拉伸
到 ROI。

## 参数

默认值全部直接读取 `AnalysisParameters()`：

| 参数 | 默认值 | GUI 范围 | 作用 |
| --- | ---: | ---: | --- |
| `resize_size` | `512` | `1..4096` | letterbox 方形画布边长，也是自适应上下文的最小边长。 |
| `context_scale` | `1.5` | `1.001..100` | bbox 最大边到请求上下文边长的倍率。 |
| `center_fraction` | `0.016` | `0.001..1` | 中心密度统计窗口相对有效内容短边的比例。 |
| `gradient_threshold` | `30` | `0..255` | 密度路径的灰度前向差分阈值。 |
| `density_low_threshold` | `0.30` | `0..1` | 滞后连通域的低阈值。 |
| `density_high_threshold` | `0.40` | `0..1` | 滞后连通域的高阈值，不得低于低阈值。 |
| `min_component_area` | `64` | `1..1000000` | 连通域最小真实像素数。 |
| `min_grad` | `20` | `0..255` | 包络路径的梯度阈值。 |
| `min_ele_area` | `64` | `1..1000000` | 包络候选最小外接框面积。 |
| `envelope_max_side_ratio` | `0.8` | `0.001..1` | 包络最长边相对有效内容最长边的上限。 |
...
│   ├── app.py             # PySide6 界面、ROI 到协议调用、单后台任务
│   ├── canvas.py          # 原图、单个 ROI overlay、ROI 编辑与缩放
