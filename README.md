...
4. 切换视图或保存 PNG。右键拖拽平移画布，`Ctrl` 加滚轮缩放；“适配窗口”不会放大超过原图尺寸。
...
GUI 和导出的分析 overlay 只显示落入所选 `pixel_bbox` 的证据。计算成功后，青色虚线
显示协议返回的 `context_bounds`，橙色实线显示 bbox；PNG 也包含这两个语义框。渲染先按 `content_bounds` 和
`valid_mask` 取出有效内容，再按 `transform.source_size` 恢复到原始
`context_bounds`，最后裁剪到 `pixel_bbox`。区域边界通过 floor/ceil 映回原图，与
`pixel_bbox` 求交后才转成 bbox 局部坐标；完整画布、padding 和 bbox 外上下文不会被
拉伸到 bbox。
...
│   ├── canvas.py          # 原图、bbox overlay、bbox 编辑、平移与缩放
